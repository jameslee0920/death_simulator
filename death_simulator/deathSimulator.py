...
This module is designed to perform the following tasks:
    ...
    2. Execute the death simulator based on the user inputs provided from the interactive
    3. Use the death simulator results to create a death certificate .pdf file

Deeath Simulator Function:
...
def death_simulator(collected_inputs_dict):
    ...
    if(curr_age <= 85):
        data_subset = deathProb.loc[(deathProb['age'] >= curr_age) &\
            (deathProb['gender'] == gender) & (deathProb['race'] == race)]
